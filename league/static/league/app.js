...
document.addEventListener('DOMContentLoaded',()=>{
 const countdown=document.querySelector('[data-countdown][data-target]');
 if(countdown){
  const target=Date.parse(countdown.dataset.target), values=['days','hours','minutes','seconds'];
  const expired=countdown.querySelector('[data-countdown-expired]');
  const update=()=>{const remaining=Math.max(0,target-Date.now());if(remaining===0){countdown.querySelector('.countdown-values').hidden=true;if(expired)expired.hidden=false;return}let seconds=Math.floor(remaining/1000);const parts={days:Math.floor(seconds/86400)};seconds%=86400;parts.hours=Math.floor(seconds/3600);seconds%=3600;parts.minutes=Math.floor(seconds/60);parts.seconds=seconds%60;values.forEach(key=>{const el=countdown.querySelector(`[data-countdown-${key}]`);if(el)el.textContent=String(parts[key]).padStart(2,'0')})};
  update();if(target>Date.now())setInterval(update,1000);
 }
 const recap=document.querySelector('[data-recap]'),share=recap?.querySelector('[data-recap-share]');if(!share)return;
 const summary=recap.querySelector('.recap-summary');
 const makeShareImage=async()=>{const season=recap.dataset.recapSeason,standing=summary.querySelector('.recap-summary-place')?.textContent||'Your season',points=summary.querySelector('.recap-lede')?.textContent||'',song=summary.querySelector('.recap-summary-grid strong')?.textContent||'',artist=summary.querySelector('.recap-summary-grid span')?.textContent||'',canvas=document.createElement('canvas'),size=1080,ctx=canvas.getContext('2d');canvas.width=size;canvas.height=size;const rounded=(x,y,w,h,r)=>{ctx.beginPath();ctx.roundRect(x,y,w,h,r);ctx.fill()};const text=(value,x,y,max,font,color)=>{ctx.font=font;ctx.fillStyle=color;const words=value.split(' ');let line='',offset=0;words.forEach(word=>{const next=(line+' '+word).trim();if(ctx.measureText(next).width>max&&line){ctx.fillText(line,x,y+offset);offset+=Math.round(parseInt(font,10)*1.15);line=word}else line=next});ctx.fillText(line,x,y+offset);return offset};const gradient=ctx.createLinearGradient(0,0,size,size);gradient.addColorStop(0,'#241044');gradient.addColorStop(.55,'#7a287d');gradient.addColorStop(1,'#ef7774');ctx.fillStyle=gradient;ctx.fillRect(0,0,size,size);ctx.fillStyle='rgba(255,223,112,.22)';ctx.beginPath();ctx.arc(930,110,250,0,Math.PI*2);ctx.fill();ctx.fillStyle='rgba(98,85,230,.46)';ctx.beginPath();ctx.arc(-50,1030,300,0,Math.PI*2);ctx.fill();ctx.fillStyle='#ffdf70';ctx.font='800 25px system-ui,-apple-system,sans-serif';ctx.fillText('QUEUEUP SEASON RECAP',72,100);text(season,72,190,850,'900 64px system-ui,-apple-system,sans-serif','#fff');ctx.font='900 144px system-ui,-apple-system,sans-serif';ctx.fillStyle='#fff';ctx.fillText(standing,72,430);ctx.font='500 34px system-ui,-apple-system,sans-serif';ctx.fillStyle='#ded8f2';ctx.fillText(points,78,485);ctx.fillStyle='rgba(255,255,255,.14)';rounded(72,560,936,225,32);ctx.fillStyle='#ffdf70';ctx.font='800 22px system-ui,-apple-system,sans-serif';ctx.fillText('SONG OF THE SEASON',108,620);text(song,108,690,820,'800 46px system-ui,-apple-system,sans-serif','#fff');if(artist)text(artist,108,750,820,'500 28px system-ui,-apple-system,sans-serif','#ded8f2');ctx.fillStyle='#060e0a';rounded(72,900,96,96,24);ctx.strokeStyle='#12d86b';ctx.lineWidth=9;ctx.lineCap='round';ctx.lineJoin='round';ctx.beginPath();ctx.moveTo(108,965);ctx.lineTo(108,920);ctx.lineTo(145,914);ctx.lineTo(145,952);ctx.stroke();ctx.fillStyle='#12d86b';ctx.beginPath();ctx.arc(97,967,12,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.arc(134,957,12,0,Math.PI*2);ctx.fill();const blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/png'));return new File([blob],'queueup-season-recap.png',{type:'image/png'})};
 let shareFile=null,shareError=false;makeShareImage().then(file=>shareFile=file).catch(()=>shareError=true);
 share.addEventListener('click',async()=>{if(!shareFile){share.textContent=shareError?'Could not prepare image':'Preparing image…';setTimeout(()=>share.textContent='Share your recap',1400);return}try{if(navigator.canShare?.({files:[shareFile]})){await navigator.share({files:[shareFile]});return}}catch(error){if(error.name==='AbortError')return}const link=document.createElement('a');link.href=URL.createObjectURL(shareFile);link.download=shareFile.name;link.click();setTimeout(()=>URL.revokeObjectURL(link.href),1000)});
});
...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 const previewModal=document.querySelector('[data-track-preview-modal]');
 if(previewModal){const title=previewModal.querySelector('[data-preview-title]'),iframe=previewModal.querySelector('[data-preview-iframe]'),close=()=>{previewModal.hidden=true;iframe.src='';iframe.removeAttribute('title');document.body.classList.remove('modal-open')};document.addEventListener('click',event=>{const button=event.target.closest('[data-track-preview]');if(!button)return;const track=button.closest('.track');const trackId=track?.querySelector('input[name=track_id]')?.value||'',safeTrackId=trackId.match(/^[A-Za-z0-9]{22}$/)?.[0];if(!safeTrackId)return;title.textContent=`Preview: ${track?.querySelector('strong')?.textContent||'Track'}`;iframe.src=`https://open.spotify.com/embed/track/${encodeURIComponent(safeTrackId)}?theme=0`;iframe.title=`Preview: ${track?.querySelector('strong')?.textContent||'Track'}`;previewModal.hidden=false;document.body.classList.add('modal-open');});previewModal.querySelectorAll('[data-preview-close]').forEach(button=>button.addEventListener('click',close));}
 const welcome=document.querySelector('[data-season-welcome]');
 if(welcome){document.body.classList.add('modal-open');welcome.querySelector('[data-season-enter]').addEventListener('click',async event=>{const button=event.currentTarget;button.disabled=true;try{const csrf=decodeURIComponent(document.cookie.match(/(?:^|;\s*)queueup_csrftoken_v2=([^;]+)/)?.[1]||''),response=await fetch(welcome.dataset.seenUrl,{method:'POST',headers:{'X-CSRFToken':csrf}});if(!response.ok)throw Error();welcome.remove();document.body.classList.remove('modal-open')}catch(_){button.disabled=false}});}
});
